...
export async function POST(request: NextRequest) {
    try {
        ...
        setTimeout(async () => {
            try {
                // Update case with analysis (already hardcoded in demo1 case.json)
                ...
                setTimeout(async () => {
                    ...
                    finalCaseData.has_case = true; // Demo1 always has a case
                ...
                console.error('Failed to complete demo1 analysis:', error);
            }
        }, 2000);

        return NextResponse.json({ success: true });
    } catch (error) {
        console.error('Failed to start demo1 analysis:', error);
        return NextResponse.json(
            { error: 'Failed to start demo1 analysis' },
            { status: 500 }
        );
    }
}
