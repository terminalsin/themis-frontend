    ...
    try {
        ...
        setTimeout(async () => {
            try {
                ...
                const memeContent = generateDemo1Meme(caseData);
                ...
                console.error('Failed to generate demo1 meme:', error);
            }
        }, 2000);

        return NextResponse.json({ success: true });
    } catch (error) {
        console.error('Failed to start demo1 meme generation:', error);
        return NextResponse.json(
            { error: 'Failed to start demo1 meme generation' },
            { status: 500 }
        );
    }
}
    ...
    try {
        ...
        if (responseType === 'response') {
            try {
                ...
                return NextResponse.json(
                    { error: 'Demo1 case data not found' },
                    { status: 404 }
                );
            }
        }
        ...
        console.error('Failed to get demo1 meme:', error);
        return NextResponse.json(
            { error: 'Demo1 meme not found' },
            { status: 404 }
        );
    }
}

function generateDemo1Meme(caseData: any): string {
    ...
    const title = hasCase ? '🎉 DEMO1 MEME! 🎉' : '😅 DEMO1 MEME 😅';
