...
export async function GET(request: NextRequest) {
    try {
        ...
        try {
            ...
            console.error('Demo2 video file not accessible:', error);
            return NextResponse.json(
                { error: 'Demo2 video file not accessible' },
                { status: 404 }
            );
        }
        ...
        if (caseData.web_video_path) {
            try {
                ...
                console.log(`📹 Using cached demo2 web video: ${videoPath}`);
            } catch {
                console.log(`⚠️ Cached demo2 web video not found, will reconvert`);
        ...
        if (videoPath === caseData.video_path) {
            try {
                ...
                console.log(`📹 Serving converted demo2 video: ${videoPath}`);

                // Update case data with new web video path
                await updateCaseWithWebVideo(caseDir, caseData.video_path, videoPath);
            } catch (conversionError) {
                console.error('Demo2 video conversion failed, serving original:', conversionError);
        ...
        console.error('Failed to get demo2 video:', error);
        return NextResponse.json(
            { error: 'Demo2 video not found' },
            { status: 404 }
        );
